...
extension Notification.Name {
    static let didReceiveURLRequest = Notification.Name("didReceiveURLRequest")
}
...
extension WebKitURLProtocol: URLSessionDataDelegate {
    ...
    public func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        ...
        if let response = response {
            NotificationCenter.default.post(name: .didReceiveURLRequest, object: nil, userInfo: ["request": request, "response": response, "responseData": responseData])
        }
